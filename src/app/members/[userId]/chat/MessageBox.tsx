...
export default function MessageBox({ message, currentUserId }: Props) {
    ...
    useEffect(() => {
        console.log('useEffect: ', messageEndRef.current)
    ...
    const renderMessageHeader = () => (
        <div className={clsx('flex items-center w-full', {
            ...
            {message.dataRead && message.recipientId !== currentUserId ? (
                <span className='text-xs text-black text-italic'>(Read {timeAgo(message.dataRead)})</span>
            ) : <div></div>}
            <div className='flex'>
